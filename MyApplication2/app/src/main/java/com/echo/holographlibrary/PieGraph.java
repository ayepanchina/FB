...
public class PieGraph extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		for (PieSlice slice : slices){
			...
			slice.setPath(p);
			slice.setRegion(new Region((int)(midX-radius), (int)(midY-radius),  (int)(midY-radius), (int)(midY+radius)));
			canvas.drawPath(p, paint);
            //if(currentSweep>90)
            //x0 + (Math.cos(angle) * radius);
            //y0 + (Math.sin(angle) * radius);
            int x=(int) (midX+((Math.cos(((360-angle)*3.14/180)))*radius));
            int y=(int)(midY+((Math.sin(((360-angle)*3.14/180)))*radius));
           // canvas.drawText(slice.getTitle(), (int) (midX+((Math.cos(((angle*3.14/180)))*radius))-5), (int)(midY+((Math.sin((angle*3.14/180)))*radius)), this.paint);
			if (indexSelected == count && listener != null){
               ...
               // canvas.drawText(slice.getTitle(), (int) (((midX+midY) / 2)), getHeight() - 5, paint);
